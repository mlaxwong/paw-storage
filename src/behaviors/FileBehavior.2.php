...
class FileBehavior extends Behavior
{
    ...
    public function afterValidate($event)
    {
        ...
        if (is_array($value)) {
            if (!isset($value['id'])) {
                $owner->addError($this->attribute, Yii::t('app', 'Wrong file format'));
            }

            if (!File::find()->andWhere(['id' => $value['id']])->exists()) {
                $owner->addError($this->attribute, Yii::t('app', "File '{id}' not found", ['id' => $value['id']]));
            }
